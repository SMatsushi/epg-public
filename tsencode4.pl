...
my ($ekey, $progNum, $meta, $svname, $NHK, $NHKsubCh);
foreach(<$res>){
    print OF $_ if ($log);
    if (/^\s*Program\s+(\d+)/) {
	$progNum=$1;
	...
	print OF "\n*** Prog=$progNum ***\n" if ($log);
	if (($vstr ne "") && ($astr ne "")) {
	    print OF " Video & Audio stream found before program change. exiting ***\n" if ($log);
	    last;
	}
    } elsif (/^\s*Metadata:/) {
	$meta = 1; next;
	...
	print OF "\n*** svname=$svname NHK=$NHK NHKsubCh=$NHKsubCh ***\n" if ($log);
    } else {
	my $tid; my $tstr;
	if($_ =~ /Stream\s+\#(\d+):(\d+)\[.+mpeg2video/) {
		...
		$tstr = "$1:$2";
		if ($NHK && $NHKsubCh != 1) {
		    print OF "\n*** Audio stream #$tstr found: not ch1 NHK.. skipped\n" if ($log);
		} else {
		    $aid = $tid; $astr = $tstr;
		    print OF "\n*** Found astr=$astr and exit.. ***\n" if ($log);
		    last;
		}
	    } elsif ($astr eq "" || $2 < $aid) {
		$tid = $2;
